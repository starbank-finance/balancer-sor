...
function _spotPriceAfterSwapTokenInForExactTokenOut(amount, poolPairData) {
    ...
    let t = getTimeTillExpiry(poolPairData.expiryTime, poolPairData.currentBlockTimestamp, poolPairData.unitSeconds);
    let Ao = amount.toNumber();
    return (0, bmath_1.bnum)(Math.pow((Math.pow(Bi, (1 - t)) + Math.pow(Bo, (1 - t)) - Math.pow((-Ao + Bo), (1 - t))), (-1 + 1 / (1 - t))) /
        Math.pow((-Ao + Bo), t) +
        Math.abs(-1 +
...
function _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(amount, poolPairData) {
    ...
    let t = getTimeTillExpiry(poolPairData.expiryTime, poolPairData.currentBlockTimestamp, poolPairData.unitSeconds);
    let Ai = amount.toNumber();
    return (0, bmath_1.bnum)(-((-((Math.pow((Math.pow(Bi, (1 - t)) - Math.pow((Ai + Bi), (1 - t)) + Math.pow(Bo, (1 - t))), (-2 + 1 / (1 - t))) *
        (-1 + 1 / (1 - t)) *
        (1 - t)) /
        Math.pow((Ai + Bi), (2 * t))) -
        Math.pow((Ai + Bi), (-1 - t)) *
...
function _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(amount, poolPairData) {
    ...
    let t = getTimeTillExpiry(poolPairData.expiryTime, poolPairData.currentBlockTimestamp, poolPairData.unitSeconds);
    let Ao = amount.toNumber();
    return (0, bmath_1.bnum)((Math.pow((Math.pow(Bi, (1 - t)) + Math.pow(Bo, (1 - t)) - Math.pow((-Ao + Bo), (1 - t))), (-2 + 1 / (1 - t))) *
        (-1 + 1 / (1 - t)) *
        (1 - t)) /
        Math.pow((-Ao + Bo), (2 * t)) +
        Math.pow((-Ao + Bo), (-1 - t)) *
