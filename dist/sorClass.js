...
function getLimitAmountSwapForPath(path, swapType) {
    ...
    if (poolPairData.length == 1) {
        ...
        if (swapType === types_1.SwapTypes.SwapExactIn) {
            ...
            let limitOutputAmountSwap1 = (0, helpersClass_1.getOutputAmountSwap)(path.pools[0], path.poolPairData[0], swapType, limitAmountSwap1);
            if (limitOutputAmountSwap1.gt(limitAmountSwap2))
                ...
                // this is necessary to avoid return NaN
                    ...
                    return (0, helpersClass_1.getOutputAmountSwap)(path.pools[0], path.poolPairData[0], types_1.SwapTypes.SwapExactOut, limitAmountSwap2);
            ...
            let limitAmountSwap1 = path.pools[0].getLimitAmountSwap(poolPairData[0], swapType);
            let limitAmountSwap2 = path.pools[1].getLimitAmountSwap(poolPairData[1], swapType);
            let limitOutputAmountSwap2 = (0, helpersClass_1.getOutputAmountSwap)(path.pools[1], path.poolPairData[1], swapType, limitAmountSwap2);
            if (limitOutputAmountSwap2.gt(limitAmountSwap1))
                // This means first hop is limiting the path
                return (0, helpersClass_1.getOutputAmountSwap)(path.pools[1], path.poolPairData[1], types_1.SwapTypes.SwapExactIn, limitAmountSwap1);
            // This means second hop is limiting the path
            else
...
const smartOrderRouter = (pools, paths, swapType, totalSwapAmount, maxPools, costReturnToken) => {
    ...
    console.log('@@@@@@sorClass: smartOrderRouter() maxPools = ', maxPools);
    console.log('@@@@@@sorClass: smartOrderRouter() paths.length = ', paths.length);
    console.log('@@@@@@sorClass: smartOrderRouter() totalSwapAmount.isZero() = ', totalSwapAmount.isZero());
    console.log('@@@@@@sorClass: smartOrderRouter() totalSwapAmount = ', totalSwapAmount);
    ...
    let highestLimitAmounts = (0, helpersClass_1.getHighestLimitAmountsForPaths)(paths, maxPools);
    console.log('@@@@@@sorClass: smartOrderRouter() highestLimitAmounts=', highestLimitAmounts);
    ...
    for (let i = 0; i < maxPools; i++) {
        console.log('@@@@@@sorClass: smartOrderRouter() 234 i=', i);
        let sumHighestLimitAmounts = highestLimitAmounts
            .slice(0, i + 1)
            .reduce((a, b) => a.plus(b));
        console.log('@@@@@@sorClass: smartOrderRouter() sumHighestLimitAmounts= ', sumHighestLimitAmounts.toString());
        console.log('@@@@@@sorClass: smartOrderRouter() totalSwapAmount= ', totalSwapAmount.toString());
        10000000;
        if (totalSwapAmount.gt(sumHighestLimitAmounts)) {
            console.log('@@@@@@sorClass: smartOrderRouter() totalSwapAmount.gt(sumHighestLimitAmounts) ');
        ...
        console.log('@@@@@@sorClass: smartOrderRouter() swapAmounts= ', swapAmounts);
        console.log('@@@@@@sorClass: smartOrderRouter() initialNumPaths= ', initialNumPaths);
        ...
        console.log('@@@@@@sorClass: smartorderRouter() difference=', difference.toString());
        console.log('@@@@@@sorClass: smartOrderRouter() sumHighestLimitAmounts= ', sumHighestLimitAmounts.toString());
        console.log('@@@@@@sorClass: smartOrderRouter() totalSwapAmount= ', totalSwapAmount.toString());
        console.log('@@@@@@sorClass: smartOrderRouter() difference= ', difference);
    ...
    for (let b = initialNumPaths; b <= paths.length; b++) {
        ...
        if (b != initialNumPaths) {
            ...
            let newSwapAmount = bignumber_1.BigNumber.min.apply(null, [
                totalSwapAmount.times((0, bmath_1.bnum)(1 / b)),
        ...
        if (b == 1)
            selectedPaths = newSelectedPaths;
        totalReturn = (0, exports.calcTotalReturn)(selectedPaths, swapType, swapAmounts);
        ...
        if (swapType === types_1.SwapTypes.SwapExactIn) {
            totalReturnConsideringFees = totalReturn.minus((0, bmath_1.bnum)(totalNumberOfPools).times(costReturnToken));
            ...
            totalReturnConsideringFees = totalReturn.plus((0, bmath_1.bnum)(totalNumberOfPools).times(costReturnToken));
            improvementCondition =
    ...
    bestPaths.forEach((path, i) => {
        ...
        if (poolPairData.length == 1) {
            ...
            returnAmount = (0, helpersClass_1.EVMgetOutputAmountSwap)(path.pools[0], poolPairData[0], swapType, swapAmount);
            console.log('@@@@@@sorClass: smartOrderRouter() 716.  returnAmount=', returnAmount);
        }
        else {
            // Multi-hop:
            ...
            if (swapType === types_1.SwapTypes.SwapExactIn) {
                ...
                amountSwap2 = (0, helpersClass_1.EVMgetOutputAmountSwap)(path.pools[0], poolPairData[0], swapType, swapAmount);
                console.log('@@@@@@sorClass: smartOrderRouter() 718. amountSwap2= ', amountSwap2);
                // Call EVMgetOutputAmountSwap to update the pool state
                // for the second hop as well (the first was updated above)
                returnAmount = (0, helpersClass_1.EVMgetOutputAmountSwap)(path.pools[1], poolPairData[1], swapType, amountSwap2);
                console.log('@@@@@@sorClass: smartOrderRouter() 718. returnAmount= ', returnAmount);
            }
            else {
                console.log('@@@@@@sorClass: smartOrderRouter() 719.  ');
                amountSwap1 = (0, helpersClass_1.EVMgetOutputAmountSwap)(path.pools[1], poolPairData[1], swapType, swapAmount);
                console.log('@@@@@@sorClass: smartOrderRouter() 719.1.  amountSwap1=', amountSwap1);
                console.log('@@@@@@sorClass: smartOrderRouter() 719.1.  swapAmount=', swapAmount);
                amountSwap2 = swapAmount;
                // Call EVMgetOutputAmountSwap to update the pool state
                // for the second hop as well (the first was updated above)
                returnAmount = (0, helpersClass_1.EVMgetOutputAmountSwap)(path.pools[0], poolPairData[0], swapType, amountSwap1);
                console.log('@@@@@@sorClass: smartOrderRouter() 719.1.  returnAmount=', returnAmount);
            }
    ...
    if (!bestTotalReturn.eq(0))
        marketSp = (0, helpersClass_1.getSpotPriceAfterSwapForPath)(largestSwapPath, swapType, bmath_1.ZERO);
    else {
        swaps = [];
        ...
        paths.forEach((path, j) => {
            ...
            if (path.limitAmount.gte(swapAmount)) {
                ...
                if (path.limitAmount.eq(swapAmount)) {
                    ...
                    effectivePrice = (0, helpersClass_1.getEffectivePriceSwapForPath)(pools, path, swapType, swapAmount);
                }
...
pathLimitAmounts, iterationCount) {
    ...
    swapAmounts.forEach((swapAmount, i) => {
        ...
        if ((iterationCount == 0 &&
            ...
            let SPaS = (0, helpersClass_1.getSpotPriceAfterSwapForPath)(path, swapType, swapAmount);
            SPaSs.push(SPaS);
            let derivative_SPaS = (0, helpersClass_1.getDerivativeSpotPriceAfterSwapForPath)(path, swapType, swapAmount);
            ...
            derivativeSPaSs.push((0, bmath_1.bnum)('NaN'));
            SPaSs.push((0, bmath_1.bnum)('NaN'));
        }
    });
    ...
    // );
    let weighted_average_SPaS = (0, bmath_1.bnum)(sumSPaSDividedByDerivativeSPaSs.toNumber() /
    ...
    swapAmounts.forEach((swapAmount, i) => {
        ...
        if ((iterationCount == 0 &&
            ...
            (iterationCount != 0 &&
                swapAmount.gt(bmath_1.ZERO) &&
                exceedingAmounts[i].lt(bmath_1.ZERO)))
            pricesForViableAmounts.push((0, helpersClass_1.getSpotPriceAfterSwapForPath)(selectedPaths[i], swapType, swapAmount));
    });
...
const calcTotalReturn = (paths, swapType, swapAmounts) => {
    ...
    paths.forEach((path, i) => {
        totalReturn = totalReturn.plus((0, helpersClass_1.getOutputAmountSwapForPath)(path, swapType, swapAmounts[i]));
    });
    return totalReturn;
};
